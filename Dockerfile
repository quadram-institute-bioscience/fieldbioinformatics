...
LABEL authors="Thanh Le Viet thanh.le-viet@quadram.ac.uk" \
      description="Docker image containing all requirements for an artic pipeline with coronahit protocol"

RUN apt-get update && apt-get install -y wget g++ git make procps samtools=1.11-1 && apt-get clean -y
